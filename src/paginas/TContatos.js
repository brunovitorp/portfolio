...
function TContatos() {
  ...
  const criarContato = () => {
    ...
    const contatoExistente = contatosFAP.find( //metodo array para busca
    ...
    if (novoContatoFAP.id) {
      ...
      axios
        ...
        .then((response) => {//resposta se bem sucedida a requisição :)
          ...
          setNovoContato({ id: "", nome: "", telefone: "",email: "", foto:""}); // Limpa os campos após salvar
        })
        .catch((error) => {
